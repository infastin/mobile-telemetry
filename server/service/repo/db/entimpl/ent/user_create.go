...
func (uc *UserCreate) AddTelemetryIDs(ids ...int) *UserCreate {
	uc.mutation.AddTelemetryIDs(ids...)
	return uc
}

// AddTelemetries adds the "telemetries" edges to the Telemetry entity.
func (uc *UserCreate) AddTelemetries(t ...*Telemetry) *UserCreate {
	ids := make([]int, len(t))
	for i := range t {
		ids[i] = t[i].ID
	}
	return uc.AddTelemetryIDs(ids...)
}
...
func (uc *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec) {
	...
	if nodes := uc.mutation.TelemetriesIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
