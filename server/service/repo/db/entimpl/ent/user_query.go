...
func (uq *UserQuery) QueryTelemetries() *TelemetryQuery {
	query := (&TelemetryClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withTelemetries: uq.withTelemetries.Clone(),
...
func (uq *UserQuery) WithTelemetries(opts ...func(*TelemetryQuery)) *UserQuery {
	query := (&TelemetryClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withTelemetries = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withTelemetries; query != nil {
		if err := uq.loadTelemetries(ctx, query, nodes,
			func(n *User) { n.Edges.Telemetries = []*Telemetry{} },
			func(n *User, e *Telemetry) { n.Edges.Telemetries = append(n.Edges.Telemetries, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadTelemetries(ctx context.Context, query *TelemetryQuery, nodes []*User, init func(*User), assign func(*User, *Telemetry)) error {
	...
	query.Where(predicate.Telemetry(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.TelemetriesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.user_telemetries
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_telemetries" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_telemetries" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
