...
type DeviceQuery struct {
	...
	withUser        *UserQuery
	withTelemetries *TelemetryQuery
...
// QueryTelemetries chains the current query on the "telemetries" edge.
func (dq *DeviceQuery) QueryTelemetries() *TelemetryQuery {
	query := (&TelemetryClient{config: dq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := dq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (dq *DeviceQuery) Clone() *DeviceQuery {
	...
	return &DeviceQuery{
		...
		withTelemetries: dq.withTelemetries.Clone(),
...
// WithTelemetries tells the query-builder to eager-load the nodes that are connected to
// the "telemetries" edge. The optional arguments are used to configure the query builder of the edge.
func (dq *DeviceQuery) WithTelemetries(opts ...func(*TelemetryQuery)) *DeviceQuery {
	query := (&TelemetryClient{config: dq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	dq.withTelemetries = query
	return dq
}
...
func (dq *DeviceQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Device, error) {
	...
	if query := dq.withUser; query != nil {
		if err := dq.loadUser(ctx, query, nodes,
			func(n *Device) { n.Edges.User = []*User{} },
			func(n *Device, e *User) { n.Edges.User = append(n.Edges.User, e) }); err != nil {
			return nil, err
		}
	}
	if query := dq.withTelemetries; query != nil {
		if err := dq.loadTelemetries(ctx, query, nodes,
			func(n *Device) { n.Edges.Telemetries = []*Telemetry{} },
			func(n *Device, e *Telemetry) { n.Edges.Telemetries = append(n.Edges.Telemetries, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (dq *DeviceQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Device, init func(*Device), assign func(*Device, *User)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(device.UserTable)
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(device.UserPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(device.UserPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(device.UserPrimaryKey[1]))
...
func (dq *DeviceQuery) loadTelemetries(ctx context.Context, query *TelemetryQuery, nodes []*Device, init func(*Device), assign func(*Device, *Telemetry)) error {
	...
	query.withFKs = true
	query.Where(predicate.Telemetry(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(device.TelemetriesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.device_telemetries
		if fk == nil {
			return fmt.Errorf(`foreign-key "device_telemetries" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "device_telemetries" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
