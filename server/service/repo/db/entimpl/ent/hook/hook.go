...
// The TelemetryFunc type is an adapter to allow the use of ordinary
// function as Telemetry mutator.
type TelemetryFunc func(context.Context, *ent.TelemetryMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f TelemetryFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.TelemetryMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.TelemetryMutation", m)
}
